...
;(function(echarts, window) {
  ...
  */
  ...
  MulDimensionChart.prototype = {
    ...
    */
    getChartOption: function() {
      ...
      var perGridHeight = realChartHeight / visualMLen,
        gridWidth = 0;
      var gridIndex = 0,
        left = LEFT_DISTANCE;
      ...
      for (i = 0; i < percentData.length; i++) {
        ...
        seriesObj = that.getSingSeriesOption({
          index: i
        });
        seriesData.push(seriesObj);
      }
    ...
    */
    getDimensionCategory: function(data, dLen, mLen, isContinue) {
      ...
      var chartLabelData = [];
      var that = this;
      var preDataItem, curDataItem, curCompareItem, preCompareItem, count = 0,
        sum = 0;
      chartData = [];
      curDataItem = data[0];
      count = 1;
    ...
    */
    getPercentData: function(chartLabelData, isContinue) {
      var categoryLabel = [];
      var sum = 0,
        labelArray, percentArray, areaArray;
      var curSum = 0,
        curBit = 0;
      var curBitArray = [];
      //分类数目
    ...
    */
    getSeriesOption: function(params) {
      ...
      var seriesObj = {
        ...
        dimensions: getDimensions(params.data),
        data: params.data
      };
      ...
      /**
       * 从第三个字段开始都为离散
       * @param {Array} data: 图表显示数据
       * @return {Array} dimensions: 维度轴的类型
       */
      function getDimensions(data) {
        var dimensions = [];
        if (data && data[0]) {
          for (var i = 0; i < data[0].length; i++) {
            i <= 1 && dimensions.push(null);
            i > 1 && dimensions.push({
              type: 'ordinal'
            });
          }
  ...
  module.exports = MulDimensionChart;
}(echarts, window));
